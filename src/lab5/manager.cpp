...
KnapsackProblemManager<Variation>::KnapsackProblemManager(std::vector<std::pair<double, double>> items, double capacity, size_t populationSize, double crossoverProbability, double mutationProbability, size_t saveBest) : items(items), capacity(capacity), populationSize(populationSize), crossoverProbability(crossoverProbability), mutationProbability(mutationProbability), saveBest(saveBest), knapsackProblem(items, capacity), geneticAlgorithm(&knapsackProblem, populationSize, crossoverProbability, mutationProbability, saveBest)
{
}
...
{
  geneticAlgorithm = genalg::GeneticAlgorithm<KnapsackProblem<Variation>>(&knapsackProblem, populationSize, crossoverProbability, mutationProbability, saveBest);
}

KnapsackProblemManagerMultiplexer::KnapsackProblemManagerMultiplexer(std::vector<std::pair<double, double>> items, double capacity, size_t populationSize, double crossoverProbability, double mutationProbability, size_t saveBest) : type(Type::Bool), boolManager(items, capacity, populationSize, crossoverProbability, mutationProbability, saveBest), intManager(items, capacity, populationSize, crossoverProbability, mutationProbability, saveBest), doubleManager(items, capacity, populationSize, crossoverProbability, mutationProbability, saveBest)
{
}
